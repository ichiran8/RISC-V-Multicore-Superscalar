    ...
    casez(state)
        IDLE : begin
            if(data_read) // busRD or busRDX
                ...
                if(cc.dREN[0] && dREN[1]) begin // basic case
        ...
        SNOOP_REQ : begin
            cc.ccsnoopaddr[!core] = {cc.daddr[core][31:2], 2'b0}; // lower 2 bits of daddr being reallocated to be used for snooping MSI 
        ...
        SNOOP_RESP : begin
            ...
            casez({cc.daddr[!core][1:0]})
                2'b0? : next_state = MEM_UPDATE_1;
                2'b10 : next_state = CACHE_UPDATE_1;
                2'b11 : next_state = CACHE_UPDATE_2;
            endcase
        end
...
assign cc.iwait[1] = ((cc.ramstate == BUSY) || (dREN[0] | dWEN[0]) && core); // core determines if this is the current core being servied
