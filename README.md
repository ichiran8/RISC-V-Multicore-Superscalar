**DISCLAIMER** For any ECE Purdue student, do not even think about attempting this design for a computer architecture class or referencing it. It will be very obvious when your design choices are tailored as the following. This design is pretty out of scope for the course (feasible but this is not covered in the course material) and you will not have the time to implement this given the course structure (WIP!). The D$ and snoopy bus are not designed to work for the processor designed in class due to the nature of supporting superscalar bandwidth! There is no module in here that will aid the any design in class so don't try. After you complete the class, you may look at this for reference on how to design a higher throughput CPU!

You will find an empty palgorithm in this directory. Any non-students can contact me for the assembly testcase but students may not!
...
3) Tournament predictor is designed with a gShare predictor and local predictor as the selector
  --> If the first instruction is a branch or jump followed by another branch or jump, the second instruction is automatically discarded, regardless of the direction of the branch
